...
// single file copy
function copyFile(file, toDir, cb,page) {
    ...
    async.waterfall([
            function (callback) {
                ...
                if(!page){
                    ...
                    var writes = fs.createWriteStream(path.join(path.dirname(toDir), path.basename(
                            file.replace(/\(\!index\)/g,page).replace(/\(\!Index\)/g,ReplaceFirstUper(page))
                        )));
                }
                ...
                reads.on("data", function (data) {
                    writes.write( data.toString().replace(/\(\!index\)/g,page).replace(/\(\!Index\)/g,ReplaceFirstUper(page)) );
                });
...
function ccoutTask(from, to, cb,page) {
    var files = [];
    var count = 1;
    ...
    wrapper.addFile = function (file, dir) {
        ...
        if(page || !/\(\!index\)/ig.test(file+dir) ){
            files.push({
                file : file,
                dir : dir
            });
...
var copyDir = function(from, to, cb,page) {
    if(!cb){
      cb=function(){};
    }
    ...
    async.waterfall([
            ...
            function (stats, callback) {
                ...
                if (stats.isFile()) {
                    ...
                    ccoutTask(from, to, callback,page);
                }
            },
            function (files, callback) {
